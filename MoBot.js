...
// WALL STREET JOURNAL
    ...
    robot.hear(/I read (.*) articles/i, function(res) {
    	...
    	if(num < 4) {
    		...
    		return res.reply("JIA YOU, NI SHI HEN BAN, What are the titles?");
    	}
    });

    var articles = [];

    robot.hear(/the titles are (.*)!/i, function(res) {
...
// RETRIEVE ARTICLES FROM DATABASE
    robot.hear(/what (.*) do i have so far?/i, function(res) {
        ...
        return  res.reply(robot.brain.get('totalArticles'));
        // return  res.reply(articles);
